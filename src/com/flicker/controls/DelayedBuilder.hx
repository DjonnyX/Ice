...
class DelayedBuilder
{
	...
	public function new(owner:FlickerControl, content:FlickerControl) {
		_owner = owner;
		_content = content;
	...
	public function start() : Void {
		...
		if (_factories.length > 0) {
			...
			if (delay > 0) {
				...
				factory = null;
				_delayedCall = cast Flicker.animator.delayCall(start, delay);
			} else {
				createWithoutDelay();
			}
		}
	...
	private function stop() : Void {
		...
		if (_delayedCall != null) {
			Flicker.animator.remove(_delayedCall);
			_delayedCall = null;
		}
	}
