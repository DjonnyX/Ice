...
import com.flicker.display.SvgSprite;
...
class Scroller extends BaseStatesControl
{
	...
	private var _forelayer:FlickerControl;
	
	/**
	 * Включает округление координат во время анимации.
	 * НЕ ВСЕГДА РАЗУМНО ВКЛЮЧАТЬ ЭТОТ ПАРАМЕТР, Т.К. ОКРУГЛЕНИЕ МОЖЕТ ДАТЬ
	 * НЕБОЛЬШОЙ СБОЙ В РАСЧЕТАХ horizontalScrollPosition/verticalScrollPosition
	 */
	...
	/**
	 * Указывает нужно ли отображать "барьеры" границ скролла или нет.
	 */
	public var enableBarriers(get, set):Bool;
	//@:getter(enableBarriers)
	private function get_enableBarriers():Bool{
		return _enableBarriers;
	}
	//@:setter(enableBarriers)
	...
	/**
	 * Задает логику скролла по странично.
	 */
	...
	public function new(?elementData:ElementData) 
	{
		...
		_content = new FlickerControl(new ElementData({'name':'content', 'interactive':false}));
		_content.onResize = resizeContent;
		addChild(_content);
		_forelayer = new FlickerControl(new ElementData({'name':'fore', 'interactive':false}));
		addChild(_forelayer);
		addEventListener(WheelScrollEvent.SCROLL, wheelScrollHandler);
		styleName = DEFAULT_STYLE;
	}
	
	public function addToContentDelayedItemFactory(factory:Function, ?owner:FlickerControl, ?content:FlickerControl) : Void {
	...
	private function pressOut(e:FingerEvent) : Void {
		...
		if (_paggination == PAGGINATION_HORIZONTAL) {
			...
			if (newX != lastX) {
				Flicker.animator.remove(_xTween);
				var durationX:Float = calculateDynamicThrowDurationByDistance(Math.abs(Math.abs(newX) - Math.abs(lastX)));
				_xTween = Flicker.animator.tween(_content, durationX, {x:newX, transitionFunc:_throwEase, onUpdate:scrollX, roundToInt:_roundToInt});
			}
		}
		if (_paggination == PAGGINATION_VERTICAL) {
			...
			if (newY != lastY) {
				...
				Flicker.animator.remove(_yTween);
				_yTween = Flicker.animator.tween(_content, 1, {y:newY, transitionFunc:_throwEase, onUpdate:scrollY, roundToInt:_roundToInt});
			}
		}
	}
	...
	private function throwScroll() : Void
	{
		if (_isDraggingHorizontally) {
			...
			if (_content._x > 0 || (rightOffset < 0 && maxScrollX > 0)) {
				Flicker.animator.remove(_xTween);
				_xTween = Flicker.animator.tween(_content, durationX, {x:0, transitionFunc:_throwEase, onUpdate:scrollX, roundToInt:_roundToInt});
			} else if (rightOffset < 0) {
				Flicker.animator.remove(_xTween);
				_xTween = Flicker.animator.tween(_content, durationX, {x:maxScrollX, transitionFunc:_throwEase, onUpdate:scrollX, roundToInt:_roundToInt});
		...
		if (_isDraggingVertically) {
			...
			if (_content._y > 0 || (bottomOffset < 0 && maxScrollY > 0)) {
				Flicker.animator.remove(_yTween);
				_yTween = Flicker.animator.tween(_content, durationY, {y:0, transitionFunc:_throwEase, onUpdate:scrollY, roundToInt:_roundToInt});
			} else if (bottomOffset < 0) {
				Flicker.animator.remove(_yTween);
				_yTween = Flicker.animator.tween(_content, durationY, {y:maxScrollY, transitionFunc:_throwEase, onUpdate:scrollY, roundToInt:_roundToInt});
	...
	private function throwScrollInertialForceX(distance:Float = null, withoutAnimation:Bool = false) : Void
	{
		...
		if (contentPos > 0 || (offset < 0 && maxScrollX > 0)) {
			...
			if (withoutAnimation) {
				...
				_xTween = Flicker.animator.tween(_content, duration / _easeBackTimeRatio, {x:0, transitionFunc:_easeOutBack, onUpdate:scrollX, roundToInt:_roundToInt});
		} else if (offset < 0) {
			if (distance != null)
			...
			if (withoutAnimation) {
				...
				_xTween = Flicker.animator.tween(_content, duration / _easeBackTimeRatio, {x:maxScrollX, transitionFunc:_easeOutBack, onUpdate:scrollX, roundToInt:_roundToInt});
		} else {
			if (distance != null)
			...
			if (withoutAnimation) {
				...
				if (_paggination == PAGGINATION_HORIZONTAL)
					...
					newX = contentPos;
				_xTween = Flicker.animator.tween(_content, duration, {x:newX, transitionFunc:_throwEase, onUpdate:scrollX, roundToInt:_roundToInt});
			}
		}
	}
	...
	private function throwScrollInertialForceY(distance:Float = null, withoutAnimation:Bool = false) : Void
	{
		...
		if (contentPos > 0 || (offset < 0 && maxScrollY > 0)) {
			...
			if (withoutAnimation) {
				...
				_yTween = Flicker.animator.tween(_content, duration / _easeBackTimeRatio, {y:0, transitionFunc:_easeOutBack, onUpdate:scrollY, roundToInt:_roundToInt});
		} else if (offset < 0) {
			if (distance != null)
				duration = calculateDynamicThrowDurationByDistance(Math.abs(offset));
			else {
			...
			if (withoutAnimation) {
				...
				_yTween = Flicker.animator.tween(_content, duration / _easeBackTimeRatio, {y:maxScrollY, transitionFunc:_easeOutBack, onUpdate:scrollY, roundToInt:_roundToInt});
		} else {
			if (distance != null)
				duration = calculateDynamicThrowDurationByDistance(Math.abs(distance));
			else
			...
			if (withoutAnimation) {
				...
				if (_paggination == PAGGINATION_VERTICAL)
					...
					newY = contentPos;
				_yTween = Flicker.animator.tween(_content, duration, {y:newY, transitionFunc:_throwEase, onUpdate:scrollY, roundToInt:_roundToInt});
			}
		}
	}
...
class Barrier extends FlickerControl
{
