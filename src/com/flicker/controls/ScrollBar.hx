...
class ScrollBar extends Scroller
{
	...
	/**
	 * roundToInt для ScrollBar всенда возвращает false
	 * Включить его невозможно. Это сделано для корректного расчета scrollPosition.
	 * @return
	 */
	private override function get_roundToInt() : Bool {
		return false;
	}
	 ...
	 * @return
	 */
	private override function set_roundToInt(v:Bool) : Bool {
		return false;
	}
	...
	private override function throwScroll() : Void
	{
		...
		if (_isDraggingHorizontally) {
			...
			if (_content.x <= 0 && _thumb.width < ts) {
				Flicker.animator.remove(_wTween);
				Flicker.animator.remove(_xTween);
				_content.x = 0;
				_wTween = Flicker.animator.tween(_thumb, durationX, {width:ts, transitionFunc:_throwEase});
				_xTween = Flicker.animator.tween(_content, durationX, {x:0, transitionFunc:_throwEase, onUpdate:scrollX});
			} else if (rightOffset > 0) {
				Flicker.animator.remove(_xTween);
				Flicker.animator.remove(_wTween);
				_wTween = Flicker.animator.tween(_thumb, durationX, {width:ts, transitionFunc:_throwEase});
				_xTween = Flicker.animator.tween(_content, durationX, {x:_width - ts, transitionFunc:_throwEase, onUpdate:scrollX});
		...
		if (_isDraggingVertically) {
			...
			if (_content.y <= 0 && _thumb.height < ts) {
				Flicker.animator.remove(_hTween);
				Flicker.animator.remove(_yTween);
				_content.y = 0;
				_hTween = Flicker.animator.tween(_thumb, durationY, {height:ts, transitionFunc:_throwEase});
				_yTween = Flicker.animator.tween(_content, durationY, {y:0, transitionFunc:_throwEase, onUpdate:scrollY});
			} else if (bottomOffset > 0) {
				Flicker.animator.remove(_yTween);
				Flicker.animator.remove(_hTween);
				_hTween = Flicker.animator.tween(_thumb, durationY, {height:ts, transitionFunc:_throwEase});
				_yTween = Flicker.animator.tween(_content, durationY, {y:_height - ts, transitionFunc:_throwEase, onUpdate:scrollY});
	...
	private override function throwScrollInertialForceX(distance:Float = null, withoutAnimation:Bool = false) : Void
	{
		...
		Flicker.animator.remove(_helpTween);
		Flicker.animator.remove(_xTween);
		Flicker.animator.remove(_wTween);
		...
		if (contentPos < 0) {
			...
			if (withoutAnimation) {
				...
				_helpTween = Flicker.animator.tween(_helperPoint, .5, {x:0, transition:Transitions.EASE_OUT_BACK, onUpdate:function () {
			...
			if (withoutAnimation) {
				...
				_helperPoint.x = _content.x;
				_helpTween = Flicker.animator.tween(_helperPoint, .5, {x:maxScrollX, transition:Transitions.EASE_OUT_BACK, onUpdate:function () {
			...
			if (withoutAnimation) {
				...
				_xTween = Flicker.animator.tween(_content, duration, {x:contentPos, transitionFunc:_throwEase, onUpdate:scrollX});
		}
	}
	...
	private override function throwScrollInertialForceY(distance:Float = null, withoutAnimation:Bool = false) : Void
	{
		...
		if (contentPos < 0) {
			...
			if (withoutAnimation) {
				...
				_helpTween = Flicker.animator.tween(_helperPoint, .5, {y:0, transition:Transitions.EASE_OUT_BACK, onUpdate:function () {
			...
			if (withoutAnimation) {
				...
				_helperPoint.y = _content.y;
				_helpTween = Flicker.animator.tween(_helperPoint, .5, {y:maxScrollY, transition:Transitions.EASE_OUT_BACK, onUpdate:function () {
			...
			if (withoutAnimation) {
				...
				_yTween = Flicker.animator.tween(_content, duration, {y:contentPos, transitionFunc:_throwEase, onUpdate:scrollY});
		}
	}
