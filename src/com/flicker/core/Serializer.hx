...
class Serializer 
{
	...
	public function serialize(data:String) : Array<FlickerControl> 
	{
		if (data == null)
			return [];
		var items:Array<FlickerControl> = [];
		var xmlData:Xml;
		try {
			xmlData = Xml.parse(data);
			var elements:Iterator<Xml> = xmlData.elements();
			for (element in elements) {
				var control:FlickerControl = parseXmlData(element);
				if (control != null)
					items.push(control);
			}
		} catch (e:Dynamic) {
	...
	private function parseXmlData(data:Xml) : FlickerControl {
		var name:String = data.nodeName.toLowerCase();
		var control:FlickerControl = parseXmlElement(data);
		if (control != null) {
			var elements:Iterator<Xml> = data.elements();
			var children:Array<FlickerControl> = [];
			for (element in elements) {
	...
	private function parseXmlElement(data:Xml) : FlickerControl {
		var element:Xml = data;
		var name:String = data.nodeName.toLowerCase();
		var control:FlickerControl = null;
		switch (name) {
			case HTML_TAG: {
	...
	private function createIceControl(data:Xml) : FlickerControl {
		var renderer:FlickerControl = new FlickerControl();
		applyIceProps(renderer, data);
		return renderer;
	}
	
	private function createIceHtmlControl(data:Xml) : FlickerControl {
	...
	private function createAnchorLayout(data:Xml) : FlickerControl {
		var renderer:FlickerControl = new FlickerControl();
		applyIceProps(renderer, data);
	...
	private function createHorizontalLayout(data:Xml) : FlickerControl {
		var renderer:FlickerControl = new FlickerControl();
		applyIceProps(renderer, data);
	...
	private function createVerticalLayout(data:Xml) : FlickerControl {
		var renderer:FlickerControl = new FlickerControl();
		applyIceProps(renderer, data);
	...
	private function createRockRowsLayout(data:Xml) : FlickerControl {
		var renderer:FlickerControl = new FlickerControl();
		applyIceProps(renderer, data);
	...
	private function applyIceProps(renderer:FlickerControl, data:Xml, fillInnerHTML:Bool = true ) : Any {
		if (fillInnerHTML) {
			var text:String = null;
			var child = data.firstChild();
			if (child != null && (child.nodeType == Xml.PCData || child.nodeType == Xml.CData)) {
